...
Graph_loc <- "./Grafiki/Homog_normu_salidzinajums/Mean_T_snht20" # Grafiku lokācijai
...
# old_dec <- read_csv2("Dati/Maksimala_gaisa_temperatura.csv")
# old_dec <- old_dec %>%
#   mutate(DATE = str_c(str_replace(format(old_dec$MON), " ", "0"),
#                       paste0("0", format(old_dec$DATE)),
#                       sep = "-")) %>%
...
normas_merge <- merge_normas(normas = list(norma22, norma33, norma77), 
                             merge_by = c("EG_GH_ID","DATE"))


# names(normas_merge)[c(3:9)] <- c("norma1", "norma2", "norma3", "norma4", "norma5", "norma6", "norma7")
names(normas_merge)[c(3:5)] <- c("norma2", "norma3", "norma7")
...
dec_norm <- merge_normas(normas = list(norma_dec22, norma_dec33, norma_dec77), 
                         merge_by = c("EG_GH_ID","DATE"))

# names(dec_norm)[c(3:9)] <- c("norma_dec1", "norma_dec2", "norma_dec3", "norma_dec4", "norma_dec5", "norma_dec6", "norma_dec7")
names(dec_norm)[c(3:5)] <- c("norma_dec2", "norma_dec3", "norma_dec7")
...
norma_mon2 <- uh_month_korig_normal
norma_mon2$DATE <- substr(paste0("0",norma_mon2$DATE), nchar(paste0("0",norma_mon2$DATE))-1 , nchar(paste0("0",norma_mon2$DATE)))
norma_mon22 <- norma_mon2 %>% gather(EG_GH_ID, VALUE, -c(DATE))

norma_mon3 <- old_mon
norma_mon3$DATE <- substr(paste0("0",norma_mon3$DATE), nchar(paste0("0",norma_mon3$DATE))-1 , nchar(paste0("0",norma_mon3$DATE)))
norma_mon33 <- norma_mon3 %>% gather(EG_GH_ID, VALUE, -c(DATE))

# norma_mon4 <- ac_mon_normal
# norma_mon4 <- norma_mon4 %>% mutate(DATE=substr(as.POSIXct(norma_mon4$DATE, format='%m-%d'),6,10))
# norma_mon44 <- norma_mon4 %>% gather(EG_GH_ID, VALUE, -c(DATE))

# norma_mon5 <- ac_mon_korig_normal
# norma_mon5 <- norma_mon5 %>% mutate(DATE=substr(as.POSIXct(norma_mon5$DATE, format='%m-%d'),6,10))
# norma_mon55 <- norma_mon5 %>% gather(EG_GH_ID, VALUE, -c(DATE))

# norma_mon6 <- clim_mon_normal
# norma_mon6 <- norma_mon6 %>% mutate(DATE=substr(as.POSIXct(norma_mon6$DATE, format='%m-%d'),6,10))
# norma_mon66 <- norma_mon6 %>% gather(EG_GH_ID, VALUE, -c(DATE))

norma_mon7 <- clim_month_korig_normal
norma_mon7$DATE <- substr(paste0("0",norma_mon7$DATE), nchar(paste0("0",norma_mon7$DATE))-1 , nchar(paste0("0",norma_mon7$DATE)))
norma_mon77 <- norma_mon7 %>% gather(EG_GH_ID, VALUE, -c(DATE))


mon_norm <- merge_normas(normas = list(norma_mon22, norma_mon33, norma_mon77), 
                         merge_by = c("EG_GH_ID","DATE"))

   
names(mon_norm)[c(3:5)] <- c("norma_mon2", "norma_mon3", "norma_mon7")
...
for(i in c(1:length(stacijas))){
  ...
  gplot <- ggmon %>% 
    ggplot(aes(x = DATE2)) +
    geom_hline(yintercept = c(0,5,10,15,20), color = "gray", size = 0.5) +
    geom_line(aes(DATE2, y = norma_mon2, group = 1, color = "UH korig norma"), size = 1) +
    geom_line(aes(DATE2, y = norma_mon3, group = 1, color = "81-10 norma"), size = 1) +
    # geom_line(aes(DATE2, y = norma_mon4, group = 1, color = "AC norma"), size = 1) + 
    # geom_line(aes(DATE2, y = norma_mon5, group = 1, color = "AC korig norma"), size = 1) + 
    # geom_line(aes(DATE2, y = norma_mon6, group = 1, color = "Cl norma"), size = 1) +
    geom_line(aes(DATE2, y = norma_mon7, group = 1, color = "Cl korig norma"), size = 1) +
    theme_classic() +
    scale_x_datetime(breaks = date_breaks(width = "1 month"),
                     labels = date_format("%1.%m.")) +
    xlab("Datums") +
  ...
  gplot +
    ggsave(paste0(Graph_loc, "/Menesi/", stac,"_mon_normas.png"))
...
html_files <- list.files(pattern = ".html")
nordir <- paste0(Graph_loc, "/Menesi")
old_html_files <- list.files(nordir, pattern = "html", full.names = T)
file.remove(old_html_files)
file.copy(html_files, nordir)
file.remove(html_files)
