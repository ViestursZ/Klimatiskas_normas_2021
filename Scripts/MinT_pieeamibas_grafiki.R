...
data <- read_rds("Dati/min_temp_dati_neapstradati.rds")
...
for (i in seq_along(stacijas)) {
  ...
  iztr_plot <- ggplot(graph_df) +
    ...
    ggtitle(paste0(stacijas[i], " minimālās gaisa temperatūras novērojumi")) +
  ...
  iztr_plot + 
    ...
    ggsave(paste0("Grafiki/Parametru_analize/Min_temperatura/", stacijas[i], ".png"))
