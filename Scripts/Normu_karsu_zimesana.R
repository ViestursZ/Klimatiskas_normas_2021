...
Graph_loc <- "./Grafiki/Homog_normu_salidzinajums/Mean_T_snht20"
...
men_norm_l <- inner_join(uh_men_korig_normal, old_norm_month_l) %>%
...
for (j in seq_along(menesi)) {
  ...
  ggplot() +
    ...
    # geom_text_repel() +
    facet_wrap(~Metode) +
    ggsave(paste0(Graph_loc, "/Menesi_kartes/Menesis_", menesi[j], ".png"),
           device = png(width = 1100, height = 700))
  dev.off()
}
...
for (j in seq_along(menesi)) {
  ...
  ggplot() +
    ...
    # geom_text_repel() +
    facet_wrap(~Metode) +
    ggsave(paste0(Graph_loc, "/Menesi_starpibas_kartes/Menesis_", menesi[j], ".png"),
           device = png(width = 1100, height = 700))
  dev.off()
}
