# Minimālās Temperatūras pieejamības grafiki
# Libraries ---------------------------------------------------------------
...
data <- read_rds("Dati/max_temp_dati_neapstradati.rds")

# Sakārto datus -----------------------------------------------------------
# Kādi regular parametri ir?
reg_pars <- c("TDRY", "PRAB", "WNS10", "HMWNS", "WPGSB", "ATMN", "ATMX")
...
# data_ur <- data_ur %>%
#   mutate(TIME = ifelse(TIME == "00:00", "MIN", TIME))
# Random testi
# data_ur %$% unique(TIME)
# 
# data_ur %>% 
...
for (i in seq_along(stacijas)) {
  ...
  iztr_plot <- ggplot(graph_df) +
    ...
    ggtitle(paste0(stacijas[i], " maksimālās gaisa temperatūras novērojumi")) +
  ...
  iztr_plot + 
    ...
    ggsave(paste0("Grafiki/Parametru_analize/Max_temperatura/", stacijas[i], ".png"))
